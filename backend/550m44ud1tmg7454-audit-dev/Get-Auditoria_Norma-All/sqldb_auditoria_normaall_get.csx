...
{
    public List<auditoria_normaall> funGetAuditoria_NormaAllList(ILogger log, int vnsearch_type
                                                    , long vnid
                                                    , int vnauditoriaid
                                                    , string vvnormacode
                                                    // , string vvcreated_by
                                                    ...
                                                    )
    {
        ...
        //Lista de Objetos
        List<auditoria_normaall> lobjs = new List<auditoria_normaall>();
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "%";   }                 
                var textselect = "SELECT * FROM auditoria.Auditoria_Norma WHERE (Deleted IS NULL OR Deleted = 0) ";
                if(vnid != 0)
                {textselect = textselect +  String.Format(" AND [Id] = {0}",vnid);} 
                if(vnauditoriaid != 0)
                {textselect = textselect +  String.Format(" AND [AuditoriaId] = {0}",vnauditoriaid);} 
                if(vvnormacode != null)
                {textselect = textselect +  String.Format(" AND [NormaCode] like '{0}'",vvcomodin+vvnormacode+vvcomodin);}
                ...
                using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
                {
                    ...
                    {   
                        ...
                        while (dataReader.Read())  
                        {
                            ...
                            if(!dataReader.IsDBNull(1)){ curobj.AuditoriaId         = (int)(dataReader.GetValue(1));}
                            if(!dataReader.IsDBNull(2)){ curobj.NormaCode           = (string)(dataReader.GetValue(2));}
        ...
        catch (Exception ex)
        {
            curobj              = new auditoria_normaall();
            curobj.Id           = 0;           
            curobj.NormaCode  = System.Convert.ToString(ex.Message);
            lobjs.Add(curobj);
        }
        return lobjs;
    }
