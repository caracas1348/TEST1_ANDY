...
{
    public List<evaluacion_auditoria_logall> funGetEvaluacion_Auditoria_LogAllList(ILogger log, int vnsearch_type
                                                    , long vnid
                                                    ...
                                                    )
    {
        ...
        //Lista de Objetos
        List<evaluacion_auditoria_logall> lobjs = new List<evaluacion_auditoria_logall>();
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "";   }                 
                var textselect = "SELECT EAL.*, SEA.Description AS DescriptionStatus ";
                textselect = textselect + "FROM [auditoria].Evaluacion_Auditoria_Log EAL ";
                textselect = textselect + "INNER JOIN [auditoria].Status_Evaluacion_Auditoria SEA ON SEA.Id = EAL.StatusEvaluacionId ";
                textselect = textselect + " WHERE 1 = 1 ";

                if(vnid != 0)
                {textselect = textselect +  String.Format(" AND EAL.[Id] = {0}",vnid);}                
                if(vnstatusevaluacionid != 0)
                {textselect = textselect +  String.Format(" AND EAL.[StatusEvaluacionId] = {0}",vnstatusevaluacionid);}
                if(vnauditoriaid != 0)
                {textselect = textselect +  String.Format(" AND EAL.[AuditoriaId] = {0}",vnauditoriaid);}                
                if(vnactive != 0)
                {textselect = textselect +  String.Format(" AND EAL.[Active] = {0}",vnactive);}
                ...
                using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
                {
                    ...
                    {   
                        ...
                        while (dataReader.Read())  
                        {
                            ...
                            if(!dataReader.IsDBNull(1)){ curobj.StatusEvaluacionId = (int)(dataReader.GetValue(1));}
                            if(!dataReader.IsDBNull(2)){ curobj.Created_By         = (string)(dataReader.GetValue(2)); }
                            if(!dataReader.IsDBNull(3)){ curobj.Created_Date       = (DateTime)(dataReader.GetValue(3));}
                            if(!dataReader.IsDBNull(4)){ curobj.Active             = (int)(dataReader.GetValue(4));}
                            if(!dataReader.IsDBNull(5)){ curobj.AuditoriaId        = (int)(dataReader.GetValue(5));}
                            if(!dataReader.IsDBNull(6)){ curobj.DescriptionStatus  = (string)(dataReader.GetValue(6));}
        ...
        catch (Exception ex)
        {
            curobj              = new evaluacion_auditoria_logall();
            curobj.Id           = 0;           
            curobj.Created_By  = System.Convert.ToString(ex.Message);
            lobjs.Add(curobj);
        }
        return lobjs;
    }
