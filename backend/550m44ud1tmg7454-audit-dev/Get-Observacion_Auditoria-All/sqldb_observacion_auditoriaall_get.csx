...
{
    public List<observacion_auditoriaall> funGetObservacion_AuditoriaAllList(ILogger log, int vnsearch_type
                                                    , int vnid
                                                    , string vvobservacion
                                                    , long vnauditoriaid
                                                    , string vvcreated_by
                                                    , int vnactive
                                                    // , string vvcreated_by
                                                    ...
                                                    )
    {
        ...
        //Lista de Objetos
        List<observacion_auditoriaall> lobjs = new List<observacion_auditoriaall>();
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "%";   }                 
                var textselect = "SELECT * FROM auditoria.Observacion_Auditoria ";
                textselect = textselect + "WHERE 1 = 1 ";
                if(vnid != 0)
                {textselect = textselect +  String.Format(" AND [Id] = {0}",vnid);}                
                if(vvobservacion != null)
                {textselect = textselect +  String.Format(" AND [Observacion] like '{0}'",vvcomodin+vvobservacion+vvcomodin);}
                if(vnauditoriaid != 0)
                {textselect = textselect +  String.Format(" AND [AuditoriaId] = {0}",vnauditoriaid);}                
                if(vvcreated_by != null)
                {textselect = textselect +  String.Format(" AND [Created_By] like '{0}'",vvcreated_by);}
                if(vnactive != 0)
                ...
                using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
                {
                    ...
                    {   
                        ...
                        while (dataReader.Read())  
                        {
                            ...
                            if(!dataReader.IsDBNull(1)){ curobj.Observacion         = (string)(dataReader.GetValue(1));}
                            if(!dataReader.IsDBNull(2)){ curobj.AuditoriaId         = (long)(dataReader.GetValue(2));}
                            if(!dataReader.IsDBNull(3)){ curobj.Created_By          = (string)(dataReader.GetValue(3));}
                            if(!dataReader.IsDBNull(5)){ curobj.Active              = (int)(dataReader.GetValue(5));}
        ...
        catch (Exception ex)
        {
            curobj              = new observacion_auditoriaall();
            curobj.Id           = 0;           
            curobj.Observacion  = System.Convert.ToString(ex.Message);
            lobjs.Add(curobj);
        }
        return lobjs;
    }
