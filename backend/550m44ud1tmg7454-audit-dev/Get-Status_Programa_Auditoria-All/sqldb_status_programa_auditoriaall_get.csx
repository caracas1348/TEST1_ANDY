...
{
    public List<status_programa_auditoriaall> funGetStatus_Programa_AuditoriaAllList(ILogger log, int vnsearch_type
                                                    , int vnid
                                                    ...
                                                    )
    {
        ...
        //Lista de Objetos
        List<status_programa_auditoriaall> lobjs = new List<status_programa_auditoriaall>();
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "%";   }                 
                var textselect = "SELECT * FROM auditoria.Status_Programa_Auditoria WHERE (Deleted IS NULL OR Deleted = 0) ";
                if(vnid != 0)
