...
{
    public List<especialidadall> funGetEspecialidadAllList(ILogger log, int vnsearch_type
                                                              , int vnid                                
                                                              , string vvcode 
                                                              , string vvdescription                                                                                                                                                                                
                                                              , string vvcreated_by
                                                              , string vvlast_updated_by
                                                              , string vvcreated_date_ini
                                                              , string vvcreated_date_end
                                                              , string vvlast_updated_date_ini
                                                              , string vvlast_updated_date_end  
                                                              )
    {
        ...
        //Lista de Objetos
        List<especialidadall> lobjs = new List<especialidadall>();
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "%";   }                 
                var textselect = "SELECT Id, Code, Description, Created_By, Created_Date, Last_Updated_By, Last_Updated_Date from auditoria.Especialidad WHERE (Deleted IS NULL OR Deleted = 0) ";
                if(vnid != null && vnid != 0)
                ...
                {textselect = textselect +  String.Format(" AND [Description] like '{0}'",vvcomodin+vvcode+vvcomodin);}
                ...
                if(vvcreated_date_ini != null)
                {textselect = textselect +  String.Format(" AND [Created_Date] >= '{0}'",vvcreated_date_ini);}
