#r "Newtonsoft.Json"
#load "sqldb_especialidadall_post.csx"
  ...
  /*START - Parametros de Lectura*/
   ...
   //Invocar INSERT
    if(vvhttpmethod == "post"){
        ...
        Task<long> curobjtask = vobj_sqldata.funPostEspecialidadAll( log , curobj);
        curobjtask.Wait();
    ...
    if(vvhttpmethod == "put"){
        ...
        curobj =  funsetObject( log , dataobject,vvhttpmethod );   
        curobj = vobj_sqldata.funPutEspecialidadAll( log ,curid, curobj);
        curobj.Id = curid;
        //Conversion de Respuestas a JSON
        jsonrspt = JsonConvert.SerializeObject(curobj, Formatting.Indented);
    }
    ...
    if(vvhttpmethod == "delete"){
        ...
        long nrorows = vobj_sqldata.funDeleteEspecialidadAll( log ,curid, usuariodelete );
...
public static especialidadall funsetObject(ILogger log, dynamic dataobject, string vvmethod)  
{
    // long vnid = 0;
    // if(vvmethod != "post")
    // {
