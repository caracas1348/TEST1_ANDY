...
class DataNormaAll
{
    public List<normaall> funGetNormaAllList(ILogger log, int vnsearch_type
                                                    , long vnid
                                                    , string vvcode
                                                    ...
                                                    )
    {
        ...
        //Lista de Objetos
        List<normaall> lobjs = new List<normaall>();
        normaall curobj;
        string vvcomodin="";
        //SQL Objects
        ...
        try{
            using (SqlConnection conn = new SqlConnection(vvsqlconnectionString))
            {
                ...
                if(vnsearch_type == 1)
                { vvcomodin= "%";   }                 
                var textselect = "SELECT N.[Id],N.[Code],N.[Description],N.[Active],N.[EspecialidadId],N.[Deleted_By],N.[Deleted_Date],N.[Deleted], ";
                textselect = textselect + "E.Code AS CodeEspecialidad, E.Description AS DescriptionEspecialidad ";
                textselect = textselect + "FROM [auditoria].[Norma] N ";
                textselect = textselect + "INNER JOIN [auditoria].Especialidad E ON E.Id = N.EspecialidadId ";
                textselect = textselect + "WHERE (N.Deleted IS NULL OR N.Deleted = 0) ";
                if(vnid != 0)
                {textselect = textselect +  String.Format(" AND N.[Id] = {0}",vnid);}                
                if(vvcode != null)
                {textselect = textselect +  String.Format(" AND N.[Code] like '{0}'",vvcomodin+vvcode+vvcomodin);}
                if(vvdescription != null)
                {textselect = textselect +  String.Format(" AND N.[Description] like '{0}'",vvcomodin+vvdescription+vvcomodin);}
                if(vnactive != 0)
                {textselect = textselect +  String.Format(" AND N.[Active] = {0}",vnactive);}
                if(vnespecialidadid != 0)
                {textselect = textselect +  String.Format(" AND N.[EspecialidadId] = {0}",vnespecialidadid);}                
                ...
                using (SqlCommand cmd = new SqlCommand(StrQuery, conn))
                {
                    ...
                    {   
                        ...
                        while (dataReader.Read())  
                        {
                            ...
                            if(!dataReader.IsDBNull(4)){ curobj.EspecialidadId      = (long)(dataReader.GetValue(4));}
                            if(!dataReader.IsDBNull(5)){ curobj.CodeEspecialidad        = (string)(dataReader.GetValue(5));}
                            if(!dataReader.IsDBNull(6)){ curobj.DescriptionEspecialidad = (string)(dataReader.GetValue(6));}
