 ...
 * };
 *
 * paginate2.init('.myTable',options,filterOptions);
 **/
...
(function(window){
    ...
    function lignePaginate(){
        ...
        var setFunctionInFilterBox = function(){
            filterSettings.filterBox.setAttribute('onkeyup','paginate2.filter()')
        }
        ...
        var pageButtons = function(numberOfPage,currentPage) {
            ...
            let capa= "<div class='d-flex justify-content-end'>";
            let inBox = "<input type='number' id='paginate_page_to_go' class='' value='"+currentPage+"' min='1' max='"+ settings.numberOfPages +"'>";
             buttons += inBox;
            let buttonNumberOfPage = "<input type='button' value='"  + numberOfPage + "' disabled>";

         /*   for (let $i=1; $i<=numberOfPage;$i++){
                if(numberOfPage > 10){
                    buttons += paginationMoreThatTenPage($i,numberOfPage);
                }else{
                    buttons += "<input type='button' id='id"+$i+"'value='"+$i+"' onclick='paginate2.sort("+$i+")'>";
                }
            }

            let nextButton = "<input type='button' value='>' class='paginate_control_next' onclick='paginate2.sort("+(currentPage + 1)+")' "+nextDisabled+">";
            buttons +=  nextButton;
        ...
        */
           ...
           let  buttonsPrevius = "<input type='button' value='<' class='paginate_control_prev' onclick='paginate2.sort("+(currentPage - 1)+")' "+prevDisabled+">";
           buttons +=  buttonsPrevius;
             let nextButton = "<input type='button' value='>' class='paginate_control_next' onclick='paginate2.sort("+(currentPage + 1)+")' "+nextDisabled+">";
             buttons +=  nextButton;
             buttons+="</div>";
            return buttons;
        ...
        var paginationMoreThatTenPage = function(iterator,numberOfPage){
            ...
            if(iterator > referenceForTheLast || iterator < 5){
                return "<input type='button' id='id"+iterator+"'value='"+iterator+"' onclick='paginate2.sort("+iterator+")'>";
            }else if((iterator + 1) == numberOfPage) {
                return middleValue + "<input type='button' id='id"+iterator+"'value='"+iterator+"' style='display: none' onclick='paginate2.sort("+iterator+")'>";
            }else {
                return "<input type='button' id='id"+iterator+"'value='"+iterator+"' style='display: none' onclick='paginate2.sort("+iterator+")'>";
            }
        }

        var addGoToPage = function(){
            let capa= "<div class='d-flex justify-content-end'>";
            let inputBox = "<input type='number' id='paginate_page_to_go' class='' value='1' min='1' max='"+ settings.numberOfPages +"'>";
           // let goButton = "<input type='button' id='paginate-go-button' value='Go' onclick='paginate2.goToPage()'>  ";
           // return inputBox + goButton;
           return capa + inputBox ;
        }
    ...
    if(typeof(window.paginate2) === 'undefined'){
        window.paginate2 = lignePaginate();
    }
})(window);