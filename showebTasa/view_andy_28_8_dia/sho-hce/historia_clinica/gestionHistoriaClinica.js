    ...
    var menupop = `<div class="dropdown">
                                          <button class=" btn-link dropdown textVertical" style='outline:none !important;' id="dropdownMenu2" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                            ...
                                            <div class="dropdown-menu">  
                                                  <a class="dropdown-item" onclick="divEditaHistoria(${Item.IdHC})"><img src="./images/sho/eyeIcon.svg" alt="" />&nbsp;&nbsp; Ver H.C.</a>
...
function cargarViewHistoria() {
  ...
  content.html(' ');
  showLoading();
  content.load('./view/sho-hce/historia_clinica/gestionHistoriaClinica.html');
}
...
function cargarViewDatosGenerales() {
  let content = $('#contentGlobal');
  content.html('');
  showLoading();
  content.load('./view/sho-hce/historia_clinica/datosGenerales.html', () => {
    cargarDatosGenerales();
    let btnToggle = $("#toggle-btn");
    let sidebar = $("#sidebar");
    let tituloMenu = $("#sidebar .titulo").text();

    // Acortando el titulo del menu
    let tituloMenuCorto = tituloMenu
      .split(" ")
      .map((e) => e[0])
      .join("");
...
function cargarDatosGenerales() {
  ...
  $.ajax(settings).done((response) => {
    ...
    if (response.HistoriaExamenAuxiliar.length > 0) {
      paObj_hc[idHC].Historia_ExamenAuxiliar = response.HistoriaExamenAuxiliar;
    ...
    console.log(response.HistoriaClin[0])
    hideLoading();
  })
}
...
function mostrarMenuTableExamen(id, nombre) {
  ...
  let archivo = '';
  paObj_hc[idHC].Historia_ExamenAuxiliar.forEach((e) => {
    if (e.Id == id) {
...
function dgSp3GuardarExamen(idEA) {
  let data = {}
  data.IdHC = idHC;
  data.IdEA = idEA;

  if (!$('#file_upload').val()) {
    return alert('archivo no cargado');
  }

  data.ArchivoBase64 = $('#file_upload').eq(0).data('file64');
  data.NombreArchivo = ($('#file_upload').val() != '') ? $('#file_upload')[0].files[0].name : 'Archivo no cargado';
  data.IdHashUser = "UsuarioIdlogeado";
  data.Conclusion = $('#dat_dg_conclusion_examen').val();
  data.FechaExamenes = $('#dat_dg_fecha_examen').val();
  data.IdTipEA = Number($('#dat_dg_tipo_examen').val());
...
function dgSp3EditarExamen(id) {
  let obj = paObj_hc[idHC].Historia_ExamenAuxiliar;
