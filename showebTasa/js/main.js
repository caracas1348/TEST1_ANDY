...
var dinamicDate = function () {
  $("#dateSystem").text(
    toCapitalize(moment().format("ddd D [de] MMM YYYY")) + ","
  );
};

$("#sesUserName").text(
  toCapitalize(getCookie("vtas_fullname" + sessionStorage.tabVisitasa))
);
moment.locale("es");
...
var vw_principal = (function () {
  var init = function () {
    ...
    $.ajax({
      ...
      url:
        apiurlsecurity +
        "/api/AuthClientCredentials?code=" +
    ...
    $("#menuAccessRetorneListExt").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/externalAccessRequestList.html",
        "externalAccessRequestList"
      );
    });
    ...
    $("#menuShowButtonNewVisita").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/auditoria/auditorList.html",
        "Lista de Auditores"
      );
    ...
    $("#menuAccessAproveCovidListColaborador").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/covidListColaboradorAprove.html",
        "covidListColaboradorAprove"
      );
      //$('body').materializeInputs();
    });
    $("#menuAccessPersonCovidListColaborador").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/covidListColaborador.html",
        "covidListColaborador"
      );
      //$('body').materializeInputs();
    });
    ...
    $("#menuAccessRequirementsProhibitions").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/requirementCondition.html",
        "requirementConditionList"
      );
    });
    ...
    $("#menuAccessCompanyExt").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/externalAccesEditCompany.html",
        "externalAccesEditCompany"
      );
    });
    $("#menuAccessPersonListExt").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/externalAccesListPerson.html",
        "externalAccesListPerson"
      );
    });
    ...
    $("#menuValidationEventList").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/validateAccessEvent.html",
        "accessIngresosEventList"
      );
    });
    ...
    $("#menuAccessProfile").click(function () {
      ...
      handlerUrlhtml(
        "contentGlobal",
        "view/externalAccessRequestEditPerson.html",
        "editProfile"
      );
    });
  };
...
//alert('comon 293 SelectIniMenu = '+SelectIniMenu);
    switch (SelectIniMenu) {
      case 1000:
        //---------------------------------------------------------------------CASO MENU.HTML-------------------------
        handlerUrlhtml(
          "contentGlobal",
          "view/auditoria/menu.html",
          "externalAccessRequestList"
        );
        break;
      //---------------------------------------------------------------------CASO MENU.HTML-------------------------
  ...
  var approveVista = function () {
    ...
    var url =
      apiurlaccessrequest +
      "/api/Post-ApproveAccessRequest-All?code=" +
      PostApproveAccessRequestAll +
      "&idaccessrequest=" +
      id +
      "&httpmethod=" +
      httpmethod +
    ...
    $.ajax({
      ...
      .fail(function (jqXHR, textStatus, errorThrown) {
        swal(
          "Error!",
          "Se produjo un problema al momento de registrar",
          "error"
        );
      });
  };
  ...
  var cancelVisita = function () {
    ...
    var url =
      apiurlaccessrequest +
      "/api/Post-CancelAccessRequest?code=" +
      PostCancelAccessRequest +
      "&idaccessrequest=" +
      id +
      "&httpmethod=" +
      httpmethod +
  ...
  var finalizeVisita = function () {
    ...
    var url =
      apiurlaccessrequest +
      "/api/Post-FinalizeAccessRequest-All?code=" +
      PostFinalizeAccessRequestAll +
      "&idaccessrequest=" +
      id +
      "&httpmethod=" +
      httpmethod +
  ...
  var denyVisita = function (razon) {
    ...
    var url =
      apiurlaccessrequest +
      "/api/Post-DenyAccessRequest?code=" +
      PostDenyAccessRequest +
      "&idaccessrequest=" +
      id +
      "&httpmethod=" +
      httpmethod +
  ...
  var reloadtableListVisita = function () {
    if (oTableVisita) oTableVisita.ajax.reload();
    else tableListVisitas();
  };
  ...
  var tableListVisitas = function (status) {
    ...
    if (!status) status = $("input:radio[name=chb_status_list]:checked").val();
    ...
    var url =
      apiurlaccessrequest +
      "/api/Get-AccessRequestFull-All?code=" +
      GetAccessRequestFullAll +
      "&search_type=" +
      search_type +
      "&httpmethod=" +
      httpmethod +
      ...
      ajax: {
        ...
        dataSrc: function (req) {
          ...
          req.map(function (item) {
            $("#cant_transportista").html(
              item.quantityrequest.cant_transportista
            );
            ...
            $("#badgelistpenDatos").text(
              item.cant_status.cant_PendientePorDatos
            );
            $("#badgelistpenApro").text(
              item.cant_status.cant_PendientePorAprobacion
            );
            ...
            $("#badgelistpenAll").text(
              parseInt(item.cant_status.cant_Progreso) +
                parseInt(item.cant_status.cant_Programado) +
                parseInt(item.cant_status.cant_PendientePorAprobacion) +
                parseInt(item.cant_status.cant_PendientePorDatos) +
                parseInt(item.cant_status.cant_Rechazado)
            );
            ...
            var authorizerPersons = item.authorized_person
              ? item.authorized_person
              : [];
            var externalcompany = item.externalcompany
              ? item.externalcompany
              : [];
            var lengthAuthorizerPerson = authorizerPersons.length;
            var cant = lengthAuthorizerPerson;
            var iconPerson = "";
            var companyname = item.authorized_person
              ? item.authorized_person[0].name_external_company
              : "-";
            var reason = item.status_id == 5 ? item.rejection_reason : "";
            if (cant <= 9 && cant > 0) cant = "0" + cant;
            //console.log(cant);
            if (lengthAuthorizerPerson > 0) {
              ...
              authorizerPersons.forEach((element) => {
                ...
                //console.log(i);
                iconPerson +=
                  '<img class="m-1" src="images/iconos/persons.svg">';
                nameParticiántes.push(
                  toCapitalize(
                    " " + element.first_mane + " " + element.last_name
                  )
                );
              });

              imgAutorizerPerson =
                '<div class="card border-0 h-0  m-0 bg-muted" style="box-shadow:0 0px 0px 0 rgba(0, 0, 0, 0.0), 0 0px 0px 0px rgba(0, 0, 0, 0), 0 0px 0px 0 rgba(0, 0, 0, 0)!important;">';
              imgAutorizerPerson +=
                '<div class="card-body p-1"><div class="row col"><h4 class="border border-top-0 border-bottom-0 border-left-0">' +
                cant +
                '</h4><span class="card-title">';
              imgAutorizerPerson += iconPerson;
              imgAutorizerPerson +=
                '</span></div><span class="card-subtitle mb-2 text-muted d-inline-block text-truncate " style="max-width: 250px;">' +
                nameParticiántes.toString() +
                "</span> </div>"; //
            ...
            if (item.enabled_mon_day) {
              colL = "badge-success";
            }
            if (item.enabled_tues_day) {
              colM = "badge-success";
            }
            if (item.enabled_wednes_days) {
              colMi = "badge-success";
            }
            if (item.enabled_thurs_day) {
              colJ = "badge-success";
            }
            if (item.enabled_fri_days) {
              colV = "badge-success";
            }
            if (item.enabled_satur_day) {
              colS = "badge-success";
            }
            if (item.enabled_sun_day) {
              colD = "badge-success";
            }
            ...
            var row = {
              ...
              //item.end_date
              reason:
                '<div data-toggle="modal" data-target="#modalShowRequestDetails" onclick="vw_access_request.viewVisita(' +
                id +
                ')" class="card border-0 h-0  m-0 bg-muted " style="cursor:pointer;box-shadow:0 0px 0px 0 rgba(0, 0, 0, 0.0), 0 0px 0px 0px rgba(0, 0, 0, 0), 0 0px 0px 0 rgba(0, 0, 0, 0)!important;"><div class="row" style="margin-left:-10px; margin-right:-10px;"> <div class="col-md-12"> <div class="card-body p-0 "><span class="card-title h6  hoverItem">' +
                toCapitalize(item.reason) +
                '</span><br><span class="card-subtitle mb-2 text-muted ">' +
                item.location_name +
                " - " +
                item.area_name +
                "</span> </div> </div></div> </div>", //
              photoCollaborator:
                '<img src="images/1-thumb.png" alt="..." class="img-thumbnail rounded-circle" >',
              collaboratorName:
                '<div class="media-body ml-3"><h6 class="mb-1 font-weight-semi-bold"><a class="text-dark " href="#">' +
                item.data_supervisor_contact +
                '</a></h6><p class="font-weight-semi-bold mb-0 text-500">' +
                item.email_responsible_contact +
                "</p></div>",
              status:
                '<i class="fa fa-circle ' +
                statusColor +
                '"></i><label style="margin-left:15px" class="text-capitalize">' +
                item.status_name +
                '</label><br><small class="text-help text-warning pl-0 text-capitalize">' +
                reason +
                "</small>",
              empresa:
                '<div class="card border-0 h-0  m-0 bg-muted " style="cursor:pointer;box-shadow:0 0px 0px 0 rgba(0, 0, 0, 0.0), 0 0px 0px 0px rgba(0, 0, 0, 0), 0 0px 0px 0 rgba(0, 0, 0, 0)!important;"><div class="row"><div class="col-md-1 mt-0"> </div> <div class="col-md-11 "> <div class="card-body p-0 "><span class="card-title h6">' +
                nameCompany
                  ? toCapitalize(nameCompany.toString())
                  : "-" + "</span> </div> </div></div> </div>", //
              participant: imgAutorizerPerson,
              detail:
                '<div data-toggle="modal" data-target="#modalShowRequestDetails" onclick="vw_access_request.viewVisita(' +
                id +
                ')"><i class="material-icons btdetail" style="cursor:pointer " >more_vert</i></div>', //'<button type="button" class="btn btn-light btn-circle btdetail" id="'+id+'" ><i class="fa fa-ellipsis-v text-secondary"></i></button>'
  ...
  var addRowTableVehiculo = function () {
    ...
    //agregamos la fila
    oTableVehiculo.row
      .add([nro, placa, marca, modelo, brevete, categoria])
      .draw(true);
    //----------
  };
  ...
  var selectTypeRequest = function (val) {
    ...
    handlerUrlhtml(
      "contentGlobal",
      "view/accessRequestRegister.html",
      "accessRequestRegister"
    );
  ...
  return {
    initList: function (tab, status) {
      initList();
      if (tab) TYPE_ACCESS_REQUEST = tab;

      if (status) selectedFilterStatus(status);
...
var selectedTextType = function (val) {
  ...
  if (val == 3) $("#listType2")[0].className = "title-1selected";
  else if (val == 2) $("#listType3")[0].className = "title-1selected";
  else $("#listType" + val)[0].className = "title-1selected";
};
...
var selectedFilterStatus = function (val) {
  ...
  if (val == 1)
    $("#label_status_list_1")[0].className =
      "btn tabInactive statusPperDatos text-white";
  else if (val == 2)
    $("#label_status_list_2")[0].className =
      "btn tabInactive statusPperAprove text-white";
  else if (val == 3)
    $("#label_status_list_3")[0].className =
      "btn tabInactive statusPperProg text-white";
  else if (val == 4)
    $("#label_status_list_4")[0].className =
      "btn tabInactive statusPperCourse text-white";
  else if (val == 5)
    $("#label_status_list_5")[0].className =
      "btn tabInactive bg-danger text-white";
  else if (val == 5)
    $("#label_status_list_6")[0].className = "btn tabInactive active";
  else if (val == 7) {
    $("#label_status_list_7")[0].className =
      "btn tabInactive statusPperCourse text-white";
    statusmed = 0;
    $("#col_observacion_vetado").hide();
  } else if (val == 8) {
    $("#label_status_list_8")[0].className =
      "btn tabInactive bg-danger text-white";
    statusmed = 1;
    ...
    if (last_veto_status == 1 || covid_test == 8) {
      //vetado o Por alta médica
      if ($("#modalRegisterCovidTestNewForm").is(":visible")) {
        //programado
        $("#divfielExtensionReposo").show();
      }
      if ($("#modalRegisterBlackCovidTest").is(":visible")) {
        //no programado
        $("#txr_divfielExtensionReposo").show();
      }
      if ($("#modalViewBlackCovidTest").is(":visible")) {
        //editar
    ...
    $("#label_altamedica_88")[0].className =
      "btn tabInactive statusPperProg text-white";
    statusmed = 88;
  } else if (val == 888) {
    $("#label_altamedica_88_nop")[0].className =
      "btn tabInactive statusPperProg text-white";
    statusmed = 88;
  } else if (val == 8888) {
    $("#label_altamedica_88_historial")[0].className =
      "btn tabInactive statusPperProg text-white";
    statusmed = 88;
  } else if (val == 9) {
    $("#label_status_list_9")[0].className =
      "btn tabInactive statusPperCourse text-white";
    statusmed = 0;
    $("#col_observacion_vetadonp").hide();
  } else if (val == 10) {
    $("#label_status_list_10")[0].className =
      "btn tabInactive bg-danger text-white";
    statusmed = 1;
    $("#col_observacion_vetadonp").show();
  } else if (val == 11) {
    $("#label_status_list_9_history")[0].className =
      "btn tabInactive statusPperCourse text-white";
    statusmed = 0;
  } else if (val == 12) {
    $("#label_status_list_10_history")[0].className =
      "btn tabInactive bg-danger text-white";
    statusmed = 1;
...
var exportPdfAlta = function () {
  ...
  var momentoActual = new Date();
  momentoActual =
    "Lima, " + moment(momentoActual).format("D [de] MMMM [del] YYYY");
  ...
  if (logoPdf) doc.addImage(logoPdf, "PNG", 80, 10, 50, 30); // EJE X,Y,WIDTH,HEIGHT
  doc.setFontType("bold");
  ...
  text += `identificado con Documento de Identidad: ${$(
    "#tx_docum"
  ).val()} realizó aislamiento social, obteniendo `;
  ...
  doc.text(140, line + 40, `${toCapitalize(momentoActual)}`, {
    align: "right",
  });
...
var exportPdfAltaHistorial = function () {
  ...
  text += `Por la presente se deja constancia que, ${$(
    "#txr_nomape_history"
  ).val()} `;
  text += `identificado con Documento de Identidad: ${$(
    "#txr_docum_history"
...
var exportPdfAltaBottonHistorial = function (
  document,
  nombre,
  attention_date,
  date_end_alatmedica_pdf,
  date_ini_alatmedica_pdf,
  health_code_cmp
) {
  var momentoActual = new Date();
  momentoActual =
  ...
  var date_init =
    date_ini_alatmedica_pdf != null && date_ini_alatmedica_pdf != "null"
      ? date_ini_alatmedica_pdf
      : new Date();
  var date_end =
    date_end_alatmedica_pdf != null && date_end_alatmedica_pdf != "null"
      ? date_end_alatmedica_pdf
      : new Date();
  health_code_cmp =
    health_code_cmp != null && health_code_cmp != "null" ? health_code_cmp : "";
...
var accceptLocations = function () {
  ...
  if ($("#listSedeSelectedUSer").val() == 0) {
    flag = 1;
    validatefield = "Debe ingresar " + "Sede";
  } else if (
    $("#listGaritaSelectedUSer").val() == 0 &&
    (getCookie("vtas_rolexternalrol" + sessionStorage.tabVisitasa) ==
      "ROL_GARITA" ||
      getCookie("vtas_rolexternalrol" + sessionStorage.tabVisitasa) ==
        "ROL_SEGURIDAD" ||
      getCookie("vtas_rolexternalrol" + sessionStorage.tabVisitasa) ==
    ...
    flag = 1;
    validatefield = "Debe ingresar " + "Garita";
  } else if (
    $("#documentSecuryin").val() == "" &&
    (getCookie("vtas_rolexternalrol" + sessionStorage.tabVisitasa) ==
      "ROL_GARITA" ||
      getCookie("vtas_rolexternalrol" + sessionStorage.tabVisitasa) ==
        "ROL_SEGURIDAD" ||
    ...
    flag = 1;
    validatefield = "Debe ingresar " + "Documento";
  }
  ...
  $.ajax({
    ...
    .done(function (data) {
      ...
      setCookie(
        "vtas_garita" + sessionStorage.tabVisitasa,
        $("#listGaritaSelectedUSer").val(),
        365
      );
      setCookie(
        "vtas_sede" + sessionStorage.tabVisitasa,
        $("#listSedeSelectedUSer").val(),
        365
      );
      setCookie(
        "vtas_sedecomp" + sessionStorage.tabVisitasa,
        $("#listSedeSelectedUSer").val() +
          "-" +
          $("#listSedeSelectedUSer  option:selected").text(),
        365
      );
...
function vistaAuditorias(a, b, c) {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/auditoriaList.html",
    "Ingreso de Auditorías - " + nombre_programa_auditoria
  );
  //$('body').materializeInputs();
}
...
function vistaEvaluarAuditorias(a, b, c) {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/auditoriasListEvaluacion.html",
    "Ingreso de Auditorías - " + nombre_programa_auditoria
  );
  //$('body').materializeInputs();
}
...
function fnSp3AdministrarHallazgos() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/AdministrarHallazgos.html",
    " Hallazgos "
  );
}
...
function fnSp3EvaluacionAuditores() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/evaluarAuditor.html",
...
function programaAuditoria() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/registrarProgramaAuditoria.dev.html",
...
function evaluaProgramaAuditoria() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/evaluacionProgramaAuditoria.html",
    "Evaluar Programa de Auditoría"
  );
}
...
function seguimientoAuditoria() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/seguimientoAuditoria.html",
    "Programa de Auditoría"
  );
}
...
function listadoAsistentes() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/listadoAsistentes.html",
...
function listadoAuditoriaLider() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/auditoriaListLiderPlan.html",
    "Auditoría de Lider"
  );
}
///////////////// @jesus
...
function fnSp4AtencionHallazgos() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/hallazgosAsignados/hallazgosAsignados.html",
    "Hallazgos Asignados"
  );
}
...
function fnSp4SeguimientoAccionesCorrectivas() {
  ...
  handlerUrlhtml(
    "contentGlobal",
    "view/auditoria/seguimientoAC/seguimientoAC.html",
...
function showxMenuRolResponsableAsignadoAC() {
  ...
  let divx = document.getElementById(
    "emergenteDivMenuRolResponsableAsignadoAC"
  );
  //var divx = document.getElementById("emergenteDivMenu");
  divx.style.zIndex = "10000";
  divx.style.visibility = "visible";
...
//############################################################################## SALUD E HIGIENE OCUPACIONAL ################################################################ AGOSTO 2021
function closexMenuSaludOcupacional() {
  var divx = document.getElementById("emergenteDivMenuResponsableAuditorias");
  divx.style.zIndex = "0";
  divx.style.visibility = "hidden";
}

function showxMenuSaludOcupacional() {
  // alert('estamos aqui.......');
  // var divx = document.getElementById("emergenteDivMenuConfiguraciones");
...
var items = $(".items-busqueda");

// Escuchar evento en el input
$inputBusqueda.keyup((e) => {
  console.log($inputBusqueda.val());
  const keyWords = $inputBusqueda.val().toLowerCase();
  items.each((item) => {
    let textItem = items.eq(item).find("span").text().toLowerCase();
    console.log(textItem);
    if (textItem.includes(keyWords)) {
      items.eq(item).show();
    } else {
      items.eq(item).hide();
    }
